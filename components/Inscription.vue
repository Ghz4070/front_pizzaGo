...
export default {
    ...
    methods: {
        async inscription() {
            ...
            const addUser = await axios.post('http://localhost:4000/api/v1/user/add', body);
            
            if(addUser.data.status == "success"){
                this.login_toast.text = "Inscription réaliser avec succées. Vous allez être redirigé vers la page d'accueil dans quelque seconde"
                this.login_toast.snackbar = true;
                setTimeout(() => {
                    this.$router.push({ name: 'index' });
                }, 1500);
            }else if(addUser.data.result === "not same password"){
                this.login_toast.text = "Vérifier qu'il sagit bien du même mot de passe"
                this.login_toast.snackbar = true;
